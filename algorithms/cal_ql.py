...
class TrainConfig(object):
    def __init__(self, 
                 ...
                 cql_target_action_gap = -1.0) -> None:
        # Experiment
        self.device: str = "cuda"
        self.env: str = "halfcheetah-medium-expert-v2"  # OpenAI gym environment name
        ...
        self.alpha_multiplier: float = alpha_multiplier  # Multiplier for alpha in loss
        self.use_automatic_entropy_tuning: bool = True  # Tune entropy
        self.backup_entropy: bool = False  # Use backup entropy
        self.policy_lr: float = 3e-5  # Policy learning rate
        ...
        self.cql_n_actions: int = cql_n_actions  # Number of sampled actions
        self.cql_importance_sample: bool = cql_importance_sample  # Use importance sampling
        ...
        self.cql_temp: float = cql_temp  # CQL temperature
        ...
        # Cal-QL
        self.mixing_ratio: float = offline_mixing_ratio  # Data mixing ratio for online tuning
        self.is_sparse_reward: bool = is_sparse_reward  # Use sparse reward
        self.log_name: str = "cal_ql_baseline"   # name for the log directory 
...
class CalQL:
    def __init__(
        ...
        self.log_alpha_prime = log_alpha_prime
        self.alpha_prime_optimizer = alpha_prime_optimizer
...
def train(args):
    ...
    if 'hopper' in env_name or 'halfcheetah' in env_name or 'walker2d' in env_name:
        ...
        gin_config_files = 'configs/cal_ql/' + env_name + '/' + other_name + '.gin'
    ...
    log_alpha_prime = Scalar(1.0)
    alpha_prime_optimizer = torch.optim.Adam(
            log_alpha_prime.parameters(),
            config.qf_lr,
        )
    ...
    kwargs = {
        ...
        "alpha_prime_optimizer": alpha_prime_optimizer,
    ...
    logger.log_str(f"Training Cal-QL, Env: {config.env}, Seed: {seed}")
    print("---------------------------------------")
    logger.log_str("---------------------------------------")
    ...
    if config.load_model != "":
        # policy_file = Path(config.load_model)
        policy_file = "logs/cal_ql/{}/{}".format(config.env, config.load_model)
        if os.path.exists(policy_file):
            trainer.load_state_dict(torch.load(policy_file))
            config.offline_iterations = 3e5
            actor = trainer.actor
...
if __name__ == "__main__":
    ...
    parser.add_argument("--env", type=str, default="halfcheetah-medium-replay-v2") ################ task
    parser.add_argument("--seed", type=int, default=10) ############################ seed
    parser.add_argument("--log_name", type=str, default="cal_ql_baseline")
