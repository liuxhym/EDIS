...
class Maze:
    ...
    def __init__(self, maze, start_cell=(0, 0), exit_cell=None, map_as_state=False, episode_length=30):
        ...
        self.__episode_length = episode_length
        ...
        self.state_dim = nrows * ncols if map_as_state else 2
        self.action_dim = 1
        self.__map_as_state = map_as_state
        self.reset(start_cell)

        self.observation_space = gym.spaces.Box(low=-np.inf, high=np.inf, shape=[self.state_dim])
        self.action_space = gym.spaces.Box(low=-np.inf, high=np.inf, shape=[self.action_dim])
    ...
    def step(self, action):
        ...
        done = status == Status.WIN or status == Status.LOSE or self.__step >= self.__episode_length
        if self.__map_as_state:
            state = self.__coord_to_map(state)
        return state, reward, done, status

    def __coord_to_map(self, state):
        return state
